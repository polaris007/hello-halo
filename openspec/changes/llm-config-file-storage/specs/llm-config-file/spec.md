...
系统 SHALL 将 `authType: 'api-key'` 的 AI 源配置存储在项目启动目录下的 `llm-config.json` 文件中。

#### Scenario: 创建新配置文件
- **WHEN** 用户首次配置自定义 API 大模型
- **AND** `llm-config.json` 文件不存在
- **THEN** 系统在项目启动目录创建 `llm-config.json` 文件
- **AND** 文件包含版本号和初始配置结构

#### Scenario: 更新配置文件
- **WHEN** 用户修改自定义 API 大模型配置
- **THEN** 系统更新 `llm-config.json` 文件
...
系统 SHALL 从 `llm-config.json` 文件读取自定义 API 大模型配置。

#### Scenario: 加载配置文件
- **WHEN** 系统需要读取大模型配置
- **AND** `llm-config.json` 文件存在
- **THEN** 系统解析 JSON 内容
- **AND** 返回配置对象

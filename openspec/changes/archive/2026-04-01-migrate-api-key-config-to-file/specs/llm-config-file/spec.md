...
系统 SHALL 将 `authType: 'api-key'` 的 AI 源配置仅存储在配置目录下的 `llm-config.json` 文件中，不再存储到数据库。
...
系统 SHALL 优先从配置目录读取 LLM 配置文件，不再从数据库读取 API-Key 配置。

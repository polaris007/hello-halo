...
- **THEN** 系统记录请求的以下信息：
  - 时间戳（本地系统时间，格式：YYYY-MM-DDTHH:mm:ss.SSS）
  - 请求方法（GET、POST等）
  - 请求URL
...
#### Scenario: 日志格式
- **WHEN** 记录HTTP请求/响应
- **THEN** 系统使用结构化文本格式记录，每条日志单行
- **AND** 格式为：`[YYYY-MM-DDTHH:mm:ss.SSS] [INFO] HTTP 请求: {method} {url} - {statusCode} - {duration}ms - {clientIp}`
- **AND** 详细信息（请求头、请求体、响应体）在后续行记录

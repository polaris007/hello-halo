 ...
 */
export function createDatabaseManager(appDbPath) {
    /**
     ...
     */
    function getCurrentVersion(db, namespace) {
        const row = db.prepare(`SELECT version FROM ${MIGRATIONS_TABLE} WHERE namespace = ?`).get(namespace);
        return row?.version ?? 0;
    }
    /**
     * Update (upsert) the schema version for a namespace.
     */
    function setVersion(db, namespace, version) {
        db.prepare(`
    ...
    const manager = {
        ...
        getSpaceDatabase(_spacePath) {
            throw new Error('[Store] Space-level databases are not implemented in V1. ' +
        ...
        runMigrations(db, namespace, migrations) {
            ...
            for (const m of migrations) {
                if (!Number.isInteger(m.version) || m.version < 1) {
                    throw new Error(`[Store] Invalid migration version ${m.version} in namespace "${namespace}". ` +
                        'Versions must be positive integers.');
                }
                if (versionSet.has(m.version)) {
                    throw new Error(`[Store] Duplicate migration version ${m.version} in namespace "${namespace}".`);
                }
            ...
            if (pending[0].version !== expectedNextVersion) {
                ...
                console.warn(`[Store] Migration gap in namespace "${namespace}": ` +
            ...
            const runMigrationsBatch = db.transaction(() => {
                for (const migration of pending) {
                    console.log(`[Store] Running migration "${namespace}" v${migration.version}: ${migration.description}`);
                    migration.up(db);
                }
            ...
            try {
                runMigrationsBatch();
                console.log(`[Store] Migrations complete for "${namespace}": ` +
                    `v${currentVersion} -> v${pending[pending.length - 1].version} ` +
                    `(${pending.length} migration${pending.length === 1 ? '' : 's'} applied)`);
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                console.error(`[Store] Migration failed for namespace "${namespace}" ` +

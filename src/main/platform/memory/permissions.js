 ...
 */
export function assertReadPermission(caller, scope) {
    if (caller.type === 'user') {
        // User sessions can read user and space memory, but NOT app memory
        if (scope === 'app') {
            throw new MemoryPermissionError(`User sessions cannot read app memory. Only the owning app can access its private memory.`);
        }
 ...
 * @throws Error with descriptive message if permission denied
 */
export function assertWritePermission(caller, scope, mode) {
    if (caller.type === 'user') {
        // User sessions can write to user and space memory (both modes)
        if (scope === 'app') {
            throw new MemoryPermissionError(`User sessions cannot write to app memory. Only the owning app can modify its private memory.`);
        }
    ...
    if (scope === 'space') {
        if (mode === 'replace') {
            throw new MemoryPermissionError(`Apps can only append to space memory, not replace it. ` +
 ...
 */
export function assertListPermission(caller, scope) {
 ...
 */
export function getReadableScopes(caller) {
    if (caller.type === 'user') {
        return ['user', 'space'];
    }
 ...
 */
export function getWritableScopes(caller) {
    if (caller.type === 'user') {
        return [

...
import { readMemoryFile, readMemoryHeadings, readMemorySection, readMemoryTail, appendToMemoryFile, replaceMemoryFile, listMemoryFiles, archiveMemoryFile, getFileSize } from './file-ops';
...
function createMemoryService() {
    const service = {
        ...
        async write(caller, params) {
            ...
            console.log(`[Memory] ${params.mode} to ${params.scope} memory ` +
                `(${params.content.length} bytes, by ${source})`);
        },
        // ── list ───────────────────────────────────────────────────────────────
        async list(caller, params) {
        ...
        async flushBeforeCompaction(_caller) {
        ...
        async compact(caller, scope) {
            ...
            console.log(`[Memory] Compacted ${scope} memory: ` +
        ...
        // ── saveSessionSummary ─────────────────────────────────────────────────
        async saveSessionSummary(caller, scope, params) {
            assertWritePermission(caller, scope, 'replace');
            const archiveDir = getMemoryArchiveDir(caller, scope);
        ...
        // ── needsCompaction ────────────────────────────────────────────────────
        async needsCompaction(caller, scope) {
            const filePath = getMemoryFilePath(caller, scope);
            const size = await getFileSize(filePath);

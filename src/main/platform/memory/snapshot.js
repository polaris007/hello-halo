 ...
 */
export async function buildMemorySnapshot(caller) {
 ...
 */
export function createMemoryStatusMcpServer(caller) {
    const memory_status = tool('memory_status', `Get structural metadata about your memory file (memory.md).\n` +
        ...
        try {
            ...
            return {
                content: [{ type: 'text', text: formatStatusResponse(snapshot) }],
            };
        }
        catch (err) {
            return {
                content: [{ type: 'text', text: `Failed to read memory status: ${err.message}` }],
                isError: true,
            };
        }
    });
 ...
 */
function formatStatusResponse(snapshot) {
    const lines = [];
    if (!snapshot.exists) {

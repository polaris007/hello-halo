...
export class SchedulerStore {
     ...
     */
    getJob(jobId) {
        const row = this.db.prepare('SELECT * FROM scheduler_jobs WHERE id = ?').get(jobId);
     ...
     */
    getEnabledJobs() {
        const rows = this.db.prepare('SELECT * FROM scheduler_jobs WHERE enabled = 1').all();
    ...
    clearStaleRunningMarkers() {
        const result = this.db.prepare(`UPDATE scheduler_jobs SET running_at_ms = NULL, status = 'idle' WHERE running_at_ms IS NOT NULL AND status = 'running'`).run();
     ...
     */
    pruneRunLog(maxEntriesPerJob = 1000) {

 ...
 */
export function parseEveryString(every) {
    const trimmed = every.trim().toLowerCase();
    const match = trimmed.match(/^(\d+(?:\.\d+)?)\s*(s|m|h|d)$/);
    if (!match) {
        throw new Error(`Invalid interval string "${every}". ` +
 ...
 */
export function computeNextRunEvery(anchorMs, everyMs, nowMs) {
 ...
 */
export function computeNextRunOnce(onceMs, nowMs) {
 ...
 */
export function computeNextRunCron(cron, timezone, nowMs) {
    const trimmed = cron.trim();
    if (trimmed.length < MIN_CRON_LENGTH) {
        throw new Error(`Invalid cron expression "${cron}": too short. ` +
 ...
 */
export function computeNextRun(schedule, anchorMs, nowMs) {
    switch (schedule.kind) {
        ...
        default: {
            ...
            throw new Error(`Unknown schedule kind: ${_exhaustive.kind}`);
        }
    }
}

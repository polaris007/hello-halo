...
export function _resetSpaceRegistry() {
    registry = null;
}
 ...
 */
function metaToEntry(meta, spacePath) {
    return {
        path: spacePath,
        name: meta.name,
        icon: meta.icon,
...
function loadSpaceIndex() {
    ...
    if (raw && raw.version === 2 && raw.spaces && typeof raw.spaces === 'object') {
        ...
        const v2Spaces = raw.spaces;
        for (const [id, v2Entry] of Object.entries(v2Spaces)) {
            if (!v2Entry || typeof v2Entry.path !== 'string')
                continue;
    ...
    const oldCustomPaths = Array.isArray(raw?.customPaths)
        ? raw.customPaths
        : [];
 ...
 */
function tryReadMeta(spacePath) {
 ...
 */
function entryToSpace(id, entry) {
    return {
        id,
        name: entry.name,
        icon: entry.icon,
 ...
 */
function entryToSpaceWithPreferences(id, entry) {
 ...
 */
export function getSpaceWithPreferences(spaceId) {
    const entry = getRegistry().get(spaceId);
    if (!entry)
        return null;
...
export function listSpaces() {
    ...
    for (const [id, entry] of getRegistry()) {
        if (entry.isTemp)
            continue; // halo-temp is returned via getHaloSpace()
 ...
 */
export function getAllSpacePaths() {
    const paths = [];
...
export function updateSpacePreferences(spaceId, preferences) {
    ...
    try {
        ...
        const existingMeta = tryReadMeta(entry.path);
        const currentPrefs = existingMeta?.preferences || {};
...
// ============================================================================
export function writeOnboardingArtifact(spaceId, fileName, content) {
...
export function saveOnboardingConversation(spaceId, userMessage, aiResponse) {
    const space = getSpace(spaceId);
    if (!space) {

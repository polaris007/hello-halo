...
function cachePut(conversationId, conversation, filePath, conversationsDir, spaceId) {
...
function cacheEvict(conversationId) {
 ...
 */
function cachedRead(spaceId, conversationId) {
    // Cache hit
 ...
 */
function cachedWrite(conversationId, conversation, filePath, conversationsDir, spaceId) {
    cachePut(conversationId, conversation, filePath, conversationsDir, spaceId);
 ...
 */
function debouncedUpdateIndexEntry(conversationsDir, spaceId, conversationId, meta) {
 ...
 */
function flushIndexWrites(conversationsDir) {
 ...
 */
export function flushAllPendingIndexWrites() {
...
// ============================================================================
function computeThoughtsSummary(thoughts) {
    const types = {};
 ...
 */
function migrateConversationV1toV2(conversationsDir, conversation) {
...
// ============================================================================
function getIndexPath(conversationsDir) {
    return join(conversationsDir, 'index.json');
}
function readIndex(conversationsDir) {
...
function writeIndex(conversationsDir, conversations) {
    const indexPath = getIndexPath(conversationsDir);
    const index = {
...
export function toggleStarConversation(spaceId, conversationId, starred) {
    ...
    const pending = pendingIndexWrites.get(conversationsDir);
    if (pending)
        pending.entries.delete(conversationId);
...
function fullScanConversations(conversationsDir, spaceId) {
...
function rebuildIndexAsync(conversationsDir, spaceId) {
    setImmediate(() => {
        try {
...
function updateIndexEntry(conversationsDir, spaceId, conversationId, meta) {
    const index = readIndex(conversationsDir);
    if (!index) {
...
// Core Functions
// ============================================================================
function getConversationsDir(spaceId) {
...
export function getMessageThoughts(spaceId, conversationId, messageId) {
    ...
    try {
        const thoughtsFile = JSON.parse(readFileSync(thoughtsPath, 'utf-8'));
...
export function saveSessionId(spaceId, conversationId, sessionId) {
    const result = cachedRead(spaceId, conversationId);
    if (!result)
        return;
    const { conversation, filePath, conversationsDir } = result;

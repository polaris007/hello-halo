...
import { navigatorWebdriverEvasion, navigatorVendorEvasion, navigatorLanguagesEvasion, navigatorHardwareConcurrencyEvasion, navigatorPluginsEvasion, navigatorPermissionsEvasion, chromeAppEvasion, chromeCsiEvasion, chromeLoadTimesEvasion, chromeRuntimeEvasion, webglVendorEvasion, mediaCodecsEvasion, iframeContentWindowEvasion, windowOuterdimensionsEvasion } from './evasions';
/**
 ...
 */
export async function injectStealthScripts(webContents) {
 ...
 */
function setupFallbackInjection(webContents, script) {
 ...
 * @param webContents - The WebContents instance to inject into
 */
export async function injectStealthScriptsOnce(webContents) {
    const script = getStealthScript();
    try {
        await webContents.executeJavaScript(script, true);

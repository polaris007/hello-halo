 ...
 */
export function getHeadlessElectronPath() {
 ...
 */
export function getWorkingDir(spaceId) {
 ...
 */
export async function getApiCredentials(config) {
 ...
 */
export async function getApiCredentialsForSource(config, sourceId, modelId) {
    const manager = getAISourceManager();
    await manager.ensureInitialized();
    const aiSources = config.aiSources;
    const source = aiSources?.version === 2
        ? aiSources.sources.find((s) => s.id === sourceId)
        : null;
    if (!source) {
 ...
 */
export function sendToRenderer(channel, spaceId, conversationId, data) {
 ...
 */
export function broadcastToAllClients(channel, data) {

...
import { getHeadlessElectronPath, getWorkingDir, getApiCredentials, getEnabledMcpServers } from './helpers';
 ...
 */
export const v2Sessions = new Map();
/**
 ...
 */
function cleanupSession(conversationId, reason, skipMapCheck = false) {
    const info = v2Sessions.get(conversationId);
    if (!info && !skipMapCheck)
        return;
...
function registerProcessExitListener(session, conversationId) {
    try {
        ...
        if (typeof transport.onExit === 'function') {
            const unsubscribe = transport.onExit((error) => {
 ...
 */
function migrateSessionIfNeeded(workDir, sessionId) {
 ...
 */
export function needsSessionRebuild(existing, newConfig) {
 ...
 */
function closeV2SessionForRebuild(conversationId) {
 ...
 */
export async function getOrCreateV2Session(spaceId, conversationId, sdkOptions, sessionId, config, workDir) {
 ...
 */
export async function ensureSessionWarm(spaceId, conversationId) {
 ...
 */
export function createSessionState(spaceId, conversationId, abortController) {
    return {
        abortController,
        spaceId,
        conversationId,
 ...
 */
export function registerActiveSession(conversationId, state) {
    activeSessions.set(conversationId, state);
}
/**
 * Unregister an active session
 */
export function unregisterActiveSession(conversationId) {
 ...
 */
export function getActiveSession(conversationId) {

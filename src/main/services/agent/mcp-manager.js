...
import { getHeadlessElectronPath, getApiCredentials, getEnabledMcpServers, inferOpenAIWireApi, broadcastToAllClients, setMainWindow } from './helpers';
...
let cachedMcpStatus = [];
let lastMcpStatusUpdate = 0;
/**
 * Get cached MCP status
 */
export function getCachedMcpStatus() {
    return cachedMcpStatus;
}
/**
 * Get last MCP status update timestamp
 */
export function getLastMcpStatusUpdate() {
 ...
 */
export function broadcastMcpStatus(mcpServers) {
 ...
 */
export function isMcpTestInProgress() {

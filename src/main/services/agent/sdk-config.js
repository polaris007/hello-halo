 ...
 */
export async function resolveCredentialsForSdk(credentials) {
 ...
 */
async function resolveAnthropicPassthrough(credentials) {
    const router = await ensureOpenAICompatRouter({ debug: false });
 ...
 */
function ensureSandboxSettings(configDir) {
    if (sandboxSettingsWritten)
        return;
 ...
 */
export function getCleanUserEnv() {
...
export function buildBaseSdkOptions(params) {
    const { credentials, workDir, electronPath, spaceId, conversationId, abortController, stderrHandler, mcpServers } = params;

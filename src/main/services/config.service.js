...
import { getBuiltinProvider } from '../../shared/constants';
 ...
 */
function migrateEncryptedCredentials() {
    ...
    if (result.migrated) {
        try {
            ...
            if (result.failures.length > 0) {
                console.warn(`[Config Migration] Failed to decrypt (cleared): ${result.failures.join(', ')}. ` +
 ...
 */
function migrateAiSourcesToV2OnDisk() {
    const configPath = getConfigPath();
    if (!existsSync(configPath)) {
        return; // No config file, nothing to migrate
    }
 ...
 */
export function getCredentialsGeneration() {
...
export function getTempSpacePath() {
    return join(getHaloDir(), 'temp');
}
export function getSpacesDir() {
 ...
 */
function migrateAiSourcesToV2(parsed) {
...
function getAiSourcesSignature(aiSources) {
    ...
    const currentConfig = legacy[current];

...
class AnalyticsService {
    ...
    async track(eventName, properties) {
        ...
        await Promise.allSettled(this.providers.map(provider => provider.track(event, this.userContext)));
    }
    /**
    ...
    async initProviders() {
        ...
        try {
            const gaProvider = createGAProvider(PROVIDER_CONFIG.ga.measurementId, PROVIDER_CONFIG.ga.apiSecret);
        ...
        console.log(`[Analytics] ${this.providers.length} provider(s) active:`, this.providers.map(p => p.name).join(', ') || 'none');
    }
    /**
     * Handle lifecycle events
     */

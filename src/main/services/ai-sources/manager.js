...
import { getCurrentSource, createEmptyAISourcesConfig, resolveLocalizedText } from '../../../shared/types';
...
function getProviderDisplayName(providerType) {
    ...
    if (provider?.displayName)
        return resolveLocalizedText(provider.displayName, app.getLocale());
    return providerType;
}
/**
...
class AISourceManager {
     ...
     */
    getBackendConfigForSource(sourceId, modelId) {
        const aiSources = this.getDecryptedAiSources();
        const source = aiSources.sources.find(s => s.id === sourceId);
        if (!source) {
    ...
    addSource(source) {
        ...
        saveConfig({ aiSources: newConfig });
    ...
    updateSource(sourceId, updates) {
        ...
        const newConfig = {
            ...aiSources,
            sources: aiSources.sources.map(s => s.id === sourceId
     ...
     */
    async startOAuthLogin(providerType) {
     ...
     */
    async completeOAuthLogin(providerType, state) {
        await this.ensureInitialized();
        const provider = this.providers.get(providerType);
        if (!provider) {
            return { success: false, error: `Unknown provider type: ${providerType}` };
     ...
     */
    async handleOAuthLoginSuccess(providerType, loginResult) {
        ...
        const existingSource = aiSources.sources.find(s => s.provider === providerType && s.authType === 'oauth');
        let newSources;
        let sourceId;
        if (existingSource) {
     ...
     */
    async ensureValidToken(sourceId) {
        const aiSources = this.getDecryptedAiSources();
        const source = aiSources.sources.find(s => s.id === sourceId);
     ...
     */
    async refreshSourceConfig(sourceId) {
        await this.ensureInitialized();
        const aiSources = this.getDecryptedAiSources();
        const source = aiSources.sources.find(s => s.id === sourceId);
        if (!source) {
...
export function getAISourceManager() {
    if (!managerInstance) {

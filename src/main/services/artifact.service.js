...
import { listArtifacts as listArtifactsCached, listArtifactsTree as listArtifactsTreeCached, loadDirectoryChildren, ensureSpaceCache, onArtifactChange } from './artifact-cache.service';
 ...
 */
export async function listArtifacts(spaceId) {
    ...
    const artifacts = cachedArtifacts.map(ca => ({
        id: ca.id,
        spaceId: ca.spaceId,
        conversationId: 'all',
        name: ca.name,
...
export function watchArtifacts(spaceId, callback) {
 ...
 */
export async function listArtifactsTree(spaceId) {
 ...
 */
export async function loadTreeChildren(spaceId, dirPath) {
 ...
 */
export async function initArtifactWatcher(spaceId) {
    const workDir = getWorkingDir(spaceId);
    if (!existsSync(workDir)) {
 ...
 */
export function subscribeToArtifactChanges(callback) {
...
export function readArtifactContent(filePath) {
 ...
 */
export function getArtifactDownloadInfo(filePath) {
    if (!existsSync(filePath)) {
        return null;
    }
    try {
 ...
 */
function detectBinaryContent(filePath) {
    try {
 ...
 */
export function saveArtifactContent(filePath, content) {

 ...
 */
export async function sendToChannel(channelType, config, payload) {
    switch (channelType) {
        case 'email': {
            const cfg = config.email;
            if (!cfg?.enabled)
                return { channel: channelType, success: false, error: 'Email channel not enabled' };
            return sendEmail(cfg, payload);
        }
        case 'wecom': {
            const cfg = config.wecom;
            if (!cfg?.enabled)
                return { channel: channelType, success: false, error: 'WeCom channel not enabled' };
            return sendWecom(cfg, payload);
        }
        case 'dingtalk': {
            const cfg = config.dingtalk;
            if (!cfg?.enabled)
                return { channel: channelType, success: false, error: 'DingTalk channel not enabled' };
            return sendDingtalk(cfg, payload);
        }
        case 'feishu': {
            const cfg = config.feishu;
            if (!cfg?.enabled)
                return { channel: channelType, success: false, error: 'Feishu channel not enabled' };
            return sendFeishu(cfg, payload);
        }
        case 'webhook': {
            const cfg = config.webhook;
            if (!cfg?.enabled)
                return { channel: channelType, success: false, error: 'Webhook channel not enabled' };
 ...
 */
export async function sendToChannels(channels, config, payload) {
    if (channels.length === 0)
        return [];
    const results = await Promise.allSettled(channels.map(ch => sendToChannel(ch, config, payload)));
 ...
 */
export async function testChannel(channelType, config) {
    ...
    try {
        ...
        switch (channelType) {
            case 'email': {
                const cfg = config.email;
                if (!cfg?.enabled)
                    return { channel: channelType, success: false, error: 'Not enabled' };

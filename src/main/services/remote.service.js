 ...
 */
export async function enableRemoteAccess(port) {
    if (isServerRunning()) {
 ...
 */
export async function disableRemoteAccess() {
    await stopTunnel();
    stopHttpServer();
    // Update config
    const config = getConfig();
 ...
 */
export function onRemoteAccessStatusChange(callback) {
 ...
 */
export async function generateQRCode(includeToken = false) {

...
export class SearchService {
     ...
     */
    async search(query, scope, currentConversationId, currentSpaceId, onProgress) {
        if (!query.trim()) {
            return [];
        }
        ...
        try {
            ...
            const promises = files.map((file, index) => this.searchFile(file, query)
                .then(results => {
                if (!this.cancelToken) {
            ...
            return mergedResults.sort((a, b) => new Date(b.messageTimestamp).getTime() - new Date(a.messageTimestamp).getTime());
        }
        catch (error) {
     ...
     */
    async searchFile(filePath, query) {
        if (this.cancelToken) {
            return [];
        }
        try {
     ...
     */
    getFilesToSearch(scope, conversationId, spaceId) {
     ...
     */
    findConversationFile(conversationId, spaceId) {
        const haloDir = getHaloDir();

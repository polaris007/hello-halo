...
import { startFallbackPolling, stopFallbackPolling, isPollingActive, onHealthEvent, emitHealthEvent, emitRendererCrash, emitRendererUnresponsive, emitAgentError } from './health-checker';
import { executeRecovery, executeRecoveryWithUI, selectRecoveryStrategy, injectSessionCleanup, canRecover, updateErrorCount } from './recovery-manager';
 ...
 */
export function initInstanceId() {
 ...
 */
export async function initializeHealthSystem() {
 ...
 */
export function setSessionCleanupFn(fn) {
...
function updateStatus(event) {
    let newStatus = systemState.status;
    if (event.category === 'critical') {
        newStatus = 'unhealthy';
    }
 ...
 */
async function attemptRecovery(strategyId) {
    try {
 ...
 */
async function attemptRecoveryWithUI(strategyId, errorMessage) {
    try {
        systemState.recoveryAttempts++;
 ...
 */
export async function triggerRecovery(strategyId, userConsented = false) {
    systemState.recoveryAttempts++;
 ...
 */
export async function triggerRecoveryWithUI(strategyId, errorMessage) {
    systemState.recoveryAttempts++;
 ...
 */
export function onAgentError(conversationId, error) {
    emitAgentError(conversationId, error);
}
/**
 * Handle process exit (called from session manager)
 */
export function onProcessExit(processId, code) {
    emitHealthEvent('process_exit', 'critical', processId, `Process exited with code ${code}`, { exitCode: code });
}
/**
 * Handle renderer crash (called from main process)
 */
export function onRendererCrash(details) {
    emitRendererCrash(details.reason);
}
/**
 * Handle renderer unresponsive (called from main process)
 */
export function onRendererUnresponsive() {
 ...
 */
export function shutdownHealthSystem() {

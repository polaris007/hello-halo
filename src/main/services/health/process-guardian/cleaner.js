 ...
 */
import { getCurrentInstanceId, getOrphanProcesses, clearOrphanEntries } from './registry';
 ...
 */
export async function isHaloManagedProcess(pid) {
    const platformOps = getPlatformOps();
    try {
 ...
 */
export async function getRunningHaloProcesses() {
    const platformOps = getPlatformOps();
    try {
        const processes = await platformOps.findByArgs(HALO_MANAGED_FLAG);
 ...
 */
function inferProcessType(commandLine) {

...
let healthChangeCallback = null;
// Last known status
let lastKnownStatus = 'healthy';
 ...
 */
export function startFallbackPolling(callback) {
    if (pollIntervalId) {
 ...
 */
async function performFallbackCheck() {
    ...
    try {
        ...
        const recentCritical = recentEvents.filter(e => e.category === 'critical' && Date.now() - e.timestamp < 60_000);
 ...
 */
async function doImmediateCheck() {
    isCheckRunning = true;
    ...
    try {
        ...
        const registeredClaudePids = registeredClaude.map(p => p.pid).filter(Boolean);
        const registeredCloudflaredPids = registeredCloudflared.map(p => p.pid).filter(Boolean);
        ...
        if (routerInfo) {
            ...
            try {
                ...
                routerStatus.responseTime = routerProbe.data?.responseTime;
                routerStatus.error = routerProbe.data?.error;
        ...
        if (serverInfo.running) {
            ...
            try {
                ...
                httpServerStatus.responseTime = httpProbe.data?.responseTime;
                httpServerStatus.error = httpProbe.data?.error;
                if (!httpProbe.healthy) {
                    issues.push(`HTTP Server not responding: ${httpServerStatus.error || 'unknown'}`);
                }
            }
            catch (error) {
                httpServerStatus.error = error.message;
 ...
 */
export async function runPpidScanAndCleanup() {
...
//# sourceMappingURL=runtime-checker.js.map
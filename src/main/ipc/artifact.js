...
import { listArtifacts, listArtifactsTree, loadTreeChildren, initArtifactWatcher, readArtifactContent, saveArtifactContent, detectFileType } from '../services/artifact.service';
// Register all artifact handlers
export function registerArtifactHandlers() {
    // List artifacts in a space (flat list for card view)
    ipcMain.handle('artifact:list', async (_event, spaceId) => {
        try {
    ...
    ipcMain.handle('artifact:list-tree', async (_event, spaceId) => {
        try {
    ...
    ipcMain.handle('artifact:load-children', async (_event, spaceId, dirPath) => {
        try {
    ...
    ipcMain.handle('artifact:init-watcher', async (_event, spaceId) => {
        try {
    ...
    ipcMain.handle('artifact:open', async (_event, filePath) => {
        try {
    ...
    ipcMain.handle('artifact:show-in-folder', async (_event, filePath) => {
        try {
    ...
    ipcMain.handle('artifact:read-content', async (_event, filePath) => {
        try {
    ...
    ipcMain.handle('artifact:save-content', async (_event, filePath, content) => {
        try {
    ...
    ipcMain.handle('artifact:detect-file-type', async (_event, filePath) => {
        try {

...
function requireRuntime() {
    ...
    if (!runtime) {
        return { success: false, error: 'App Runtime is not yet initialized. Please try again shortly.' };
    }
...
export function registerAppHandlers() {
    // ── app:install ──────────────────────────────────────────────────────────
    ipcMain.handle('app:install', async (_event, input) => {
        try {
    ...
    // ── app:uninstall ────────────────────────────────────────────────────────
    ipcMain.handle('app:uninstall', async (_event, input) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    // ── app:reinstall ──────────────────────────────────────────────────────
    ipcMain.handle('app:reinstall', async (_event, input) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    // ── app:delete ─────────────────────────────────────────────────────────
    ipcMain.handle('app:delete', async (_event, input) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    // ── app:list ─────────────────────────────────────────────────────────────
    ipcMain.handle('app:list', async (_event, filter) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    // ── app:get ──────────────────────────────────────────────────────────────
    ipcMain.handle('app:get', async (_event, appId) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    // ── app:pause ────────────────────────────────────────────────────────────
    ipcMain.handle('app:pause', async (_event, appId) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    // ── app:resume ───────────────────────────────────────────────────────────
    ipcMain.handle('app:resume', async (_event, appId) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    // ── app:trigger ──────────────────────────────────────────────────────────
    ipcMain.handle('app:trigger', async (_event, appId) => {
        try {
    ...
    // ── app:get-state ────────────────────────────────────────────────────────
    ipcMain.handle('app:get-state', async (_event, appId) => {
        try {
            const r = requireRuntime();
            if (!r.success)
                return r;
    ...
    // ── app:get-activity ─────────────────────────────────────────────────────
    ipcMain.handle('app:get-activity', async (_event, input) => {
        try {
            const r = requireRuntime();
            if (!r.success)
                return r;
    ...
    ipcMain.handle('app:respond-escalation', async (_event, input) => {
        try {
            const r = requireRuntime();
            if (!r.success)
                return r;
    ...
    // ── app:update-config ────────────────────────────────────────────────────
    ipcMain.handle('app:update-config', async (_event, input) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    ipcMain.handle('app:update-frequency', async (_event, input) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    ipcMain.handle('app:update-overrides', async (_event, input) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    // ── app:update-spec ──────────────────────────────────────────────────────
    ipcMain.handle('app:update-spec', async (_event, input) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    ipcMain.handle('app:grant-permission', async (_event, input) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    ipcMain.handle('app:revoke-permission', async (_event, input) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    // ── app:get-session ──────────────────────────────────────────────────────
    ipcMain.handle('app:get-session', async (_event, input) => {
        try {
            const r = requireManager();
            if (!r.success)
                return r;
    ...
    // ── app:chat-send ─────────────────────────────────────────────────────
    ipcMain.handle('app:chat-send', async (_event, request) => {
        try {
    ...
    // ── app:chat-stop ──────────────────────────────────────────────────────
    ipcMain.handle('app:chat-stop', async (_event, appId) => {
        try {
    ...
    // ── app:chat-status ────────────────────────────────────────────────────
    ipcMain.handle('app:chat-status', async (_event, appId) => {
        try {
            return {
                success: true,
                data: {
    ...
    // ── app:chat-messages ──────────────────────────────────────────────────
    ipcMain.handle('app:chat-messages', async (_event, input) => {
        try {
    ...
    ipcMain.handle('app:chat-session-state', async (_event, appId) => {
        try {
    ...
    // ── app:export-spec ────────────────────────────────────────────────────
    ipcMain.handle('app:export-spec', async (_event, appId) => {
        try {
    ...
    // ── app:import-spec ────────────────────────────────────────────────────
    ipcMain.handle('app:import-spec', async (_event, input) => {
        try {
            const result = await appController.importSpec(input);
            if (result.success) {
                console.log(`[AppIPC] app:import-spec: appId=${result.data?.appId}, space=${input.spaceId}`);
            }
            return result;
        }
        catch (error) {

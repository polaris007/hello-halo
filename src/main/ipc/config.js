...
export function registerConfigHandlers() {
        ...
        try {
            ...
            if (decryptedConfig.aiSources?.version === 2 && Array.isArray(decryptedConfig.aiSources.sources)) {
                decryptedConfig.aiSources = {
                    ...decryptedConfig.aiSources,
                    sources: decryptedConfig.aiSources.sources.map((source) => ({
                        ...source,
    ...
    ipcMain.handle('config:set', async (_event, updates) => {
        ...
        const incomingAiSources = updates.aiSources;
        ...
        if (incomingAiSources?.sources) {
            ...
            console.log('[Settings] config:set - Total sources:', incomingAiSources.sources.length, 'names:', incomingAiSources.sources.map(s => s.name).join(', '));
        }
        try {
    ...
    ipcMain.handle('config:validate-api', async (_event, apiKey, apiUrl, provider, model) => {
    ...
    ipcMain.handle('config:fetch-models', async (_event, apiKey, apiUrl) => {
    ...
    ipcMain.handle('config:refresh-ai-sources', async () => {
        ...
        try {
            ...
            const config = getConfig();
            console.log('[Settings] config:refresh-ai-sources - Refreshed, current:', config.aiSources?.current || 'custom');
            return { success: true, data: config };
        }
        catch (error) {
            const err = error;
    ...
    ipcMain.handle('ai-sources:switch-source', async (_event, sourceId) => {
    ...
    ipcMain.handle('ai-sources:set-model', async (_event, modelId) => {
    ...
    // Add new source
    ipcMain.handle('ai-sources:add-source', async (_event, source) => {
    ...
    ipcMain.handle('ai-sources:update-source', async (_event, sourceId, updates) => {
    ...
    // Delete source
    ipcMain.handle('ai-sources:delete-source', async (_event, sourceId) => {

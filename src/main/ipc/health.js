...
import { getHealthStatus, getHealthState, triggerRecovery, collectDiagnosticReport, formatReportAsText, exportReport, runImmediateCheck } from '../services/health';
/**
...
export function registerHealthHandlers() {
    ...
    ipcMain.handle('health:trigger-recovery', async (_event, strategyId, userConsented) => {
        console.log('[Settings] health:trigger-recovery - Strategy:', strategyId, 'consented:', userConsented);
        try {
            const result = await triggerRecovery(strategyId, userConsented);
    ...
    ipcMain.handle('health:export-report', async (_event, filePath) => {

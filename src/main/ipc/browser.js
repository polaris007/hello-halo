 ...
 */
export function registerBrowserHandlers(mainWindow) {
    if (!mainWindow) {
     ...
     */
    ipcMain.handle('browser:create', async (_event, { viewId, url }) => {
     ...
     */
    ipcMain.handle('browser:destroy', async (_event, { viewId }) => {
        try {
     ...
     */
    ipcMain.handle('browser:show', async (_event, { viewId, bounds }) => {
     ...
     * Hide a BrowserView
     */
    ipcMain.handle('browser:hide', async (_event, { viewId }) => {
        try {
     ...
     */
    ipcMain.handle('browser:resize', async (_event, { viewId, bounds }) => {
        try {
     ...
     * Navigate to a URL
     */
    ipcMain.handle('browser:navigate', async (_event, { viewId, url }) => {
        try {
     ...
     * Go back in history
     */
    ipcMain.handle('browser:go-back', async (_event, { viewId }) => {
        try {
     ...
     */
    ipcMain.handle('browser:go-forward', async (_event, { viewId }) => {
        try {
     ...
     * Reload the page
     */
    ipcMain.handle('browser:reload', async (_event, { viewId }) => {
        try {
     ...
     * Stop loading
     */
    ipcMain.handle('browser:stop', async (_event, { viewId }) => {
        try {
     ...
     * Get current state
     */
    ipcMain.handle('browser:get-state', async (_event, { viewId }) => {
        try {
     ...
     * Capture screenshot
     */
    ipcMain.handle('browser:capture', async (_event, { viewId }) => {
        try {
     ...
     * Execute JavaScript
     */
    ipcMain.handle('browser:execute-js', async (_event, { viewId, code }) => {
        try {
     ...
     * Set zoom level
     */
    ipcMain.handle('browser:zoom', async (_event, { viewId, level }) => {
        try {
     ...
     * Toggle DevTools
     */
    ipcMain.handle('browser:dev-tools', async (_event, { viewId }) => {
        try {
     ...
     */
    ipcMain.handle('browser:show-context-menu', async (_event, options) => {
     ...
     * Uses Electron Menu.popup() which renders above BrowserView
     */
    ipcMain.handle('canvas:show-tab-context-menu', async (_event, options) => {

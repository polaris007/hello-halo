...
export function registerAuthHandlers() {
     ...
     */
    ipcMain.handle('auth:start-login', async (_event, providerType) => {
        try {
     ...
     */
    ipcMain.handle('auth:complete-login', async (_event, providerType, state) => {
        try {
     ...
     */
    ipcMain.handle('auth:refresh-token', async (_event, sourceId) => {
        try {
     ...
     */
    ipcMain.handle('auth:check-token', async (_event, sourceId) => {
        try {
            const result = await manager.ensureValidToken(sourceId);
            if (result.success) {

...
import { readCachedIndex, writeCachedIndex, readCachedSpec, writeCachedSpec, clearCache, } from './registry.cache';
// ============================================
// Constants
// ============================================
...
const APP_TYPE_VALUES = ['automation', 'skill', 'mcp', 'extension'];
// ============================================
...
const slugToRegistryMap = new Map();
// ============================================
 ...
 */
export function initRegistryService(overrides) {
    if (initialized) {
 ...
 */
export function shutdownRegistryService() {
    if (!initialized)
        return;
    saveConfigToFile();
    indexCache.clear();
 ...
 */
export async function refreshIndex(registryId) {
    ensureInitialized();
    ...
    const results = await Promise.allSettled(registries.map(registry => fetchAndCacheIndex(registry)));
 ...
 */
function resolveEntrySearchText(entry, locale) {
 ...
 */
export async function getAppDetail(slug) {
    ensureInitialized();
 ...
 */
export async function installFromStore(slug, spaceId, userConfig) {
    ensureInitialized();
    // Find the entry
    const entries = await getIndex();
    const entry = entries.find(e => e.slug === slug);
    ...
    if (!isBundleFormat(entry)) {
        throw new Error(`This app uses legacy package format "${String(entry.format)}". Bundle packages are required in this build.`);
    }
    // Resolve the registry ID for this entry
    const registryId = resolveRegistryId(entry);
    // Fetch the full spec
 ...
 */
export async function checkUpdates(installedApps) {
    ensureInitialized();
    const mergedEntries = await getIndex();
    const updates = [];
    const registryEntriesCache = new Map();
    const getEntriesForRegistry = async (registryId) => {
 ...
 */
export function addRegistry(registry) {
    ensureInitialized();
 ...
 */
export function removeRegistry(registryId) {
    ensureInitialized();
 ...
 */
export function toggleRegistry(registryId, enabled) {
    ensureInitialized();
    const registry = config.registries.find(r => r.id === registryId);
    if (!registry) {
        throw new Error(`Registry not found: ${registryId}`);
...
export function loadConfig() {
    try {
        ...
        const storeConfig = haloConfig[CONFIG_KEY];
        if (!storeConfig) {
            return {
                registries: [DEFAULT_REGISTRY],
                cacheTtlMs: DEFAULT_CACHE_TTL_MS,
        ...
        return {
            registries: normalizeRegistries(Array.isArray(storeConfig.registries)
                ? storeConfig.registries
                : [DEFAULT_REGISTRY]),
            cacheTtlMs: normalizeCacheTtl(typeof storeConfig.cacheTtlMs === 'number'
...
function parseNumericDotVersion(version) {
    return version
        .trim()
        .replace(/^v/i, '')
        .split('.')
...
function withInstallStoreMetadata(spec, slug, registryId) {
    return {
        ...spec,
        store: {
            ...(spec.store ?? {}),
 ...
 */
function getValidCachedIndex(registryId) {
    const now = Date.now();
...
async function loadIndexForRegistry(registry) {
 ...
 */
async function fetchAndCacheIndex(registry) {
    ...
    try {
        ...
        if (!parsed.success) {
            throw new Error(`Invalid index format: ${parsed.error.issues.map(issue => issue.path.join('.')).join(', ') || 'schema mismatch'}`);
        }
 ...
 */
function resolveRegistryId(entry) {
 ...
 */
async function fetchSpec(entry, registryId) {
    ...
    try {
        ...
        if (parsedSpec.type !== entry.type) {
            throw new Error(`Spec type mismatch for "${entry.slug}": index=${entry.type}, spec=${parsedSpec.type}`);
        }
        if (parsedSpec.store?.slug && parsedSpec.store.slug !== entry.slug) {
            throw new Error(`Spec slug mismatch for "${entry.slug}": index=${entry.slug}, spec=${parsedSpec.store.slug}`);
        }

 ...
 */
import { BaseStreamHandler, OPENAI_CHAT_STOP_REASON_MAP } from './base-stream-handler';
...
export class OpenAIChatStreamHandler extends BaseStreamHandler {
    ...
    processToolCalls(toolCalls) {
        ...
        for (const toolCall of toolCalls) {
            ...
            if (processedIndices.has(toolIndex))
                continue;
 ...
 */
export async function streamOpenAIChatToAnthropic(stream, res, model, debug = false) {
    const handler = new OpenAIChatStreamHandler(res, { model, debug });
    await handler.processStream(stream);
}
//# sourceMappingURL=openai-chat-stream.js.map
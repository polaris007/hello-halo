 ...
 */
import { convertAnthropicToOpenAIChat, convertAnthropicToOpenAIResponses, convertOpenAIChatToAnthropic, convertOpenAIResponsesToAnthropic } from '../converters';
 ...
 */
function getUpstreamError(status, errorText) {
    try {
 ...
 */
function sendError(res, errorType, message) {
 ...
 */
async function fetchAnthropicUpstream(targetUrl, apiKey, bodyOrBuffer, timeoutMs, sdkHeaders, customHeaders) {
    const controller = new AbortController();
    const timeout = setTimeout(() => {
 ...
 */
function forwardResponseHeaders(upstreamResp, res) {
 ...
 */
async function handleAnthropicPassthrough(anthropicRequest, config, res, options) {
    ...
    const fetchBody = canUseRawBody ? rawBody : anthropicRequest;
    if (debug) {
        console.log(`[RequestHandler] Raw body forwarding: ${canUseRawBody ? 'yes' : 'no (modified)'}`);
    }
    try {
        const upstreamResp = await fetchAnthropicUpstream(targetUrl, apiKey, fetchBody, timeoutMs, sdkHeaders, customHeaders);
 ...
 */
async function handleOpenAIConversion(anthropicRequest, config, res, options) {
    ...
    await withRequestQueue(queueKey, async () => {
        try {
            ...
            const adapter = applyProviderAdapter(backendUrl, openaiRequest, requestHeaders);
            if (adapter && debug) {
            ...
            if (!upstreamResp.ok) {
                ...
                if (requiresStream && !wantStream) {
                    ...
                    applyProviderAdapter(backendUrl, retryRequest, requestHeaders);
 ...
 */
export async function handleMessagesRequest(anthropicRequest, config, res, options = {}) {
    ...
    const interceptResult = await runInterceptors(anthropicRequest, { originalModel: anthropicRequest.model, res });
 ...
 */
export function handleCountTokensRequest(messages, system) {
    let count = 0;

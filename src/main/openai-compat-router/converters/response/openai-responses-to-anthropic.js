 ...
 */
export function mapStatusToStopReason(status) {
    if (!status)
        return 'end_turn';
    ...
    if (Array.isArray(obj.content)) {
        const textParts = obj.content
            ...
            if (typeof c === 'object' && c !== null) {
                const cObj = c;
                if (cObj.text)
                    return String(cObj.text);
                if (cObj.content)
                    return String(cObj.content);
            }
            return '';
        })
            .filter(Boolean);
 ...
 */
export function convertOpenAIResponsesToAnthropic(openaiResponse) {
    if (!openaiResponse) {
...
//# sourceMappingURL=openai-responses-to-anthropic.js.map
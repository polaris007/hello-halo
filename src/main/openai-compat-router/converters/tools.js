 ...
 */
export function anthropicToolToOpenAIChatTool(tool) {
    return {
        type: 'function',
        function: {
            name: tool.name,
 ...
 */
export function anthropicToolToResponsesTool(tool) {
    return {
        type: 'function',
        name: tool.name,
        description: tool.description || '',
 ...
 */
export function convertAnthropicToolsToOpenAIChat(tools) {
 ...
 */
export function convertAnthropicToolsToResponses(tools) {
    if (!Array.isArray(tools) || tools.length === 0) {
        return undefined;
    }
    return tools
 ...
 */
export function convertAnthropicToolChoiceToOpenAIChat(toolChoice) {
    if (!toolChoice)
        return undefined;
 ...
 */
export function convertAnthropicToolChoiceToResponses(toolChoice) {
    if (!toolChoice)
        return undefined;
    switch (toolChoice.type) {
        case 'auto':
 ...
 */
export function budgetTokensToReasoningEffort(budgetTokens) {
    if (!budgetTokens)
        return 'medium';
 ...
 */
export function convertAnthropicThinkingToOpenAIReasoning(thinking) {
    if (!thinking)
        return undefined;
    return {
 ...
 */
export function convertAnthropicThinkingToResponsesReasoning(thinking) {
    if (!thinking)
        return undefined;
    return {
        enabled: thinking.type === 'enabled',

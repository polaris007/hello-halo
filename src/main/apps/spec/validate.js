 ...
 */
export function validateAppSpec(parsed) {
    try {
 ...
 */
export function validateAppSpecSafe(parsed) {
    try {
        ...
        return {
            success: false,
            error: new AppSpecValidationError(err instanceof Error ? err.message : String(err), [{ path: '', message: String(err) }])
        };
    }
}
/**

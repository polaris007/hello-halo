 ...
 */
const durationString = z.string().regex(/^\d+[smhd]$/, 'Duration must be a number followed by s/m/h/d (e.g. "30m", "2h", "1d")');
/**
...
}).refine((data) => Boolean(data.every) || Boolean(data.cron), { message: 'Schedule source requires either "every" or "cron"' });
...
export const StoreMetadataSchema = z.object({
    ...
    slug: z.string().regex(/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/, 'Slug must be lowercase alphanumeric with optional internal hyphens').optional(),
...
export const AppSpecBaseSchema = z.object({
    ...
    i18n: z.record(z.string(), z.object({
        ...
        description: z.string().optional(),
        config_schema: z.record(z.string(), z.object({

 ...
 */
export function normalizeRawSpec(raw) {
    const spec = { ...raw };
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    const existingRequires = (spec.requires ?? {});
    ...
    if (Array.isArray(spec.subscriptions)) {
        spec.subscriptions = spec.subscriptions.map((sub) => {
            if (sub && typeof sub === 'object' && !Array.isArray(sub)) {
                ...
                if (s.type !== undefined && s.source === undefined) {
                    ...
                    if (frequency !== undefined)
                        normalizedSub.frequency = frequency;
                    // Accept `input` as alias for `config_key`
                    if (config_key !== undefined)
                        normalizedSub.config_key = config_key;
                    else if (input !== undefined)
                        normalizedSub.config_key = input;

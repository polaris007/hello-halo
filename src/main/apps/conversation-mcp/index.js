...
// ============================================
function buildTools(spaceId) {
    const list_automation_apps = tool('list_automation_apps', 'List all automation apps installed in the current space. Returns app ID, name, description, status, and schedule.', {}, async () => {
        try {
            ...
            const lines = apps.map(app => `- ID: ${app.id} | Name: ${app.spec.name} | Status: ${app.status} | Description: ${app.spec.description}`);
            return textResult(lines.join('\n'));
        }
        catch (e) {
            return textResult(`Error listing apps: ${e.message}`, true);
        }
    });
    const create_automation_app = tool('create_automation_app', 'Create and install a new automation app (digital human) in the current space. ' +
        ...
        spec: z.string().describe('JSON string of the App Spec object. Must include name, description, system_prompt, and subscriptions. ' +
        ...
        try {
            ...
            try {
                ...
                return textResult(`Spec validation failed: ${e.message}`, true);
            }
            ...
            if (runtime) {
                try {
                    ...
                    activationWarning = ` Warning: activation failed: ${e.message}`;
                }
            }
            return textResult(`App created successfully. ID: ${appId}.${activationWarning}`);
        }
        catch (e) {
            return textResult(`Error creating app: ${e.message}`, true);
        }
    });
    const delete_automation_app = tool('delete_automation_app', 'Permanently delete an automation app. This stops the app and removes all its data.', {
        ...
        try {
            ...
            if (runtime) {
                try {
                    ...
                    deactivateWarning = ` Warning: deactivation failed: ${e.message}`;
                }
            }
            ...
            return textResult(`Error deleting app: ${e.message}`, true);
        }
    });
    const get_automation_status = tool('get_automation_status', 'Get the full details of an automation app, including its complete spec (system_prompt, subscriptions, ' +
        ...
        try {
            ...
            return textResult(`Error getting app status: ${e.message}`, true);
        }
    });
    const pause_automation_app = tool('pause_automation_app', 'Pause an active automation app. It will stop running on schedule until resumed.', {
        ...
        try {
            ...
            return textResult(`Error pausing app: ${e.message}`, true);
        }
    });
    const resume_automation_app = tool('resume_automation_app', 'Resume a paused automation app. It will run on schedule again.', {
        ...
        try {
            ...
            return textResult(`Error resuming app: ${e.message}`, true);
        }
    });
    const update_automation_app = tool('update_automation_app', 'Update an existing automation app using JSON Merge Patch semantics.\n\n' +
        ...
        updates: z.string().describe('JSON string of fields to update. Only include fields you want to change. ' +
        ...
        try {
            ...
            try {
                ...
                return textResult(`Invalid JSON in updates: ${e.message}`, true);
            }
            ...
            const frequencyShorthand = updates.frequency;
            ...
            if (frequencyShorthand && typeof frequencyShorthand === 'string') {
                ...
                if (scheduleSub) {
                    ...
                    const newSub = {
                        source: { type: 'schedule', config: { every: frequencyShorthand } }
                    };
            ...
            try {
                ...
                return textResult(`Update failed: ${e.message}`, true);
            }
            ...
            if (runtime && userChangedSubscriptions) {
                ...
                try {
                    ...
                    return textResult(`App spec updated, but runtime reactivation failed: ${e.message}. ` +
            ...
            return textResult(`Error updating app: ${e.message}`, true);
        }
    });
    const trigger_automation_app = tool('trigger_automation_app', 'Manually trigger an automation app to run immediately, regardless of its schedule. ' +
        ...
        try {
            ...
            if (e instanceof ConcurrencyLimitError && e.isPerApp) {
                ...
                return textResult(`App ${args.app_id} is already running or queued. ` +
                    `Only one execution per app is allowed at a time. ` +
                    `Please wait for the current run to complete before triggering again.`);
            }
            return textResult(`Error triggering app: ${e.message}`, true);
        }
    });
    return [
 ...
 */
export function createHaloAppsMcpServer(spaceId) {

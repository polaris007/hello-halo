 ...
 */
export function createNotifyToolServer(context) {
    const sendNotification = tool('send_notification', 'Send a notification to an external channel (email, WeCom, DingTalk, Feishu, or webhook). ' +
        ...
        channel: z.enum(['email', 'wecom', 'dingtalk', 'feishu', 'webhook']).describe('Target notification channel. Must be one of the configured and enabled channels.'),
        title: z.string().describe('Notification title. Keep it short and descriptive (e.g. "Price Alert", "Report Ready").'),
        body: z.string().describe('Notification body. The main content to deliver. Write for humans — be clear and direct.'),
    }, async (input) => {
        const runTag = context.runId.slice(0, 8);
        console.log(`[Runtime][${runTag}] send_notification called: channel=${input.channel}, title="${input.title}"`);
        // Read current config
        let config;
        try {
            config = getConfig();
        ...
        if (!channelsConfig) {
            return textResult('No notification channels configured. The user needs to set up channels in Settings > Notification Channels first.', true);
        }
        // Check if the requested channel is enabled
        const channelType = input.channel;
        const channelConfig = channelsConfig[channelType];
        if (!channelConfig?.enabled) {
            return textResult(`The "${input.channel}" channel is not enabled. Available channels: ${getEnabledChannels(channelsConfig).join(', ') || 'none'}. ` +
                'The user needs to enable it in Settings > Notification Channels.', true);
        }
    ...
    const listChannels = tool('list_notification_channels', 'List all notification channels configured by the user and their enabled status. ' +
        'Call this before send_notification to check which channels are available.', {}, async () => {
        const runTag = context.runId.slice(0, 8);

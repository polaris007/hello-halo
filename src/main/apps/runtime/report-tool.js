 ...
 */
export function createReportToolServer(store, runContext, onEscalation) {
    const reportTool = tool('report_to_user', 'Write an entry to the Activity Thread so the user knows what happened. ' +
        ...
        type: z.enum([
            ...
            'escalation',
            'output',
        ]).describe('Entry type. Use "run_complete" for normal task completion. REQUIRED — must be one of the listed values.'),
        summary: z.string().describe('REQUIRED. The actual content to show the user. Write what happened, be specific. ' +
            'Example: "💧 Drink water reminder: Stay hydrated! It\'s been 1 hour since your last reminder." ' +
            'Do NOT leave this empty or generic.'),
        data: z.record(z.unknown()).optional().describe('Optional structured data (tables, lists) for rich rendering.'),
        question: z.string().optional().describe('Only for escalation: the question to ask the user.'),
        choices: z.array(z.string()).optional().describe('Only for escalation: preset answer choices (user can also type freely).'),
    }, async (input) => {
        ...
        const VALID_TYPES = ['run_complete', 'run_skipped', 'milestone', 'escalation', 'output'];
        const safeType = VALID_TYPES.includes(input.type)
            ? input.type
            : 'run_complete';
        const safeSummary = input.summary ?? 'Task completed.';
        const summaryPreview = safeSummary.slice(0, 80);
        console.log(`[Runtime][${runTag}] report_to_user called: type=${safeType}${input.type !== safeType ? ` (original: ${input.type})` : ''}, ` +
        ...
        broadcastToAll('app:activity_entry:new', { appId: runContext.appId, entry: entry });
        ...
        if (safeType === 'escalation') {
            ...
            return textResult(`Escalation sent to user (entry: ${entryId}). ` +

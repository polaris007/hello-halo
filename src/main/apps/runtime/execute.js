...
import { openSessionWriter } from './session-store';
// ============================================
// Constants
// ============================================
...
const AUTO_CONTINUE_MESSAGE = 'You ended your response without calling report_to_user. ' +
...
const AUTO_CONTINUE_FINAL_MESSAGE = 'FINAL REMINDER: You must call report_to_user NOW. ' +
...
export async function executeRun(options) {
    ...
    console.log(`[Runtime][${runTag}] ▶ Starting run: app=${app.id}, trigger=${trigger.type}, ` +
    ...
    try {
        ...
        console.log(`[Runtime][${runTag}] Credentials resolved: provider=${credentials.provider}, ` +
        ...
        console.log(`[Runtime][${runTag}] Memory scope: type=${memoryScope.type}, spaceId=${memoryScope.spaceId}, ` +
        ...
        console.log(`[Runtime][${runTag}] ── SYSTEM PROMPT ──────────────────────────\n` +
            systemPrompt +
        ...
        console.log(`[Runtime][${runTag}] Memory snapshot: exists=${memorySnapshot.exists}, ` +
        ...
        console.log(`[Runtime][${runTag}] ── INITIAL MESSAGE ────────────────────────\n` +
            initialMessage +
        ...
        const reportMcpServer = createReportToolServer(store, reportContext, (entryId) => {
        ...
        console.log(`[Runtime][${runTag}] Creating V2 session: workDir=${workDir}, ` +
            `promptLen=${systemPrompt.length}, maxTurns=${MAX_TURNS}, ` +
            `mcpServers=[${mcpServerNames.join(', ')}], aiBrowser=${usesAIBrowser}`);
        session = await unstable_v2_createSession(sdkOptions);
        ...
        while (!streamResult.reportToolCalled &&
            ...
            console.log(`[Runtime][${runTag}] ⟳ Auto-continue #${autoContinueCount}/${MAX_AUTO_CONTINUES}: ` +
            ...
            const nextResult = await processStream(session, continueMessage, abortController, runTag, sessionWriter);
        ...
        if (autoContinueCount > 0) {
            console.log(`[Runtime][${runTag}] Auto-continue finished: attempts=${autoContinueCount}, ` +
        ...
        if (escalationEntryId) {
            ...
            finalStatus = 'error';
            outcome = 'error';
            console.warn(`[Runtime][${runTag}] AI never called report_to_user after ` +
        ...
        console.log(`[Runtime][${runTag}] ✓ Run completed: outcome=${outcome}, status=${finalStatus}, ` +
 ...
 */
async function processStream(session, message, abortController, runTag, writer) {
    const tag = runTag || '????';
    const result = {
        finalText: '',
        totalTokens: 0,
    ...
    try {
        for await (const sdkMessage of session.stream()) {
                ...
                if (Array.isArray(content)) {
                    for (const block of content) {
                        if (block.type === 'thinking' && typeof block.thinking === 'string') {
                            console.log(`[Runtime][${tag}] ── THINKING ───────────────────────────────\n` +
                                block.thinking +
                        ...
                        if (block.type === 'text' && typeof block.text === 'string') {
                            ...
                            if (block.text.trim()) {
                                console.log(`[Runtime][${tag}] ── AI TEXT ────────────────────────────────\n` +
                                    block.text +
                        ...
                        if (block.type === 'tool_use') {
                            ...
                            console.log(`[Runtime][${tag}] ── TOOL CALL ──────────────────────────────\n` +
                                `  name:  ${block.name}\n` +
                                `  id:    ${block.id || '(none)'}\n` +
                ...
                if (Array.isArray(content)) {
                    for (const block of content) {
                        if (block.type === 'tool_result') {
                            ...
                            console.log(`[Runtime][${tag}] ── TOOL RESULT ────────────────────────────\n` +
            ...
            if (msgType === 'result') {
                ...
                console.log(`[Runtime][${tag}] Stream result: tokens=${result.totalTokens}, ` +
            ...
            if (msgType === 'system') {
                if (sdkMessage.subtype === 'init') {
                    ...
                    console.log(`[Runtime][${tag}] Session initialized: ${sdkMessage.session_id || 'unknown'}\n` +
                        `  tools: [${tools.join(', ')}]\n` +
        ...
        if (abortController.signal.aborted) {
            ...
            throw new RunExecutionError('unknown', 'unknown', streamErr instanceof Error ? streamErr.message : String(streamErr));
        }
    }
    console.log(`[Runtime][${tag}] Stream finished: messages=${messageCount}, toolCalls=${toolUseCount}, ` +
 ...
 */
async function saveRunSessionSummary(memory, scope, ctx) {
 ...
 */
function buildSummaryContent(ctx) {
    const lines = [];
...
async function checkAndCompactMemory(memory, scope, app, runTag) {
    try {
        ...
        console.log(`[Runtime][${runTag}] Memory compacted: ` +
 ...
 */
function isValidCompaction(content) {
    return /^# now\s*$/m.test(content) && /^# History\s*$/m.test(content);
}
/** Build the compaction prompt for the LLM */
function buildCompactionPrompt(content, appName) {
    return (`You are compacting the memory file for an automation app called "${appName}".\n\n` +
...
async function generateCompactionSummary(content, appName, app, runTag) {
    try {
        ...
        for (let attempt = 0; attempt <= COMPACTION_MAX_RETRIES; attempt++) {
            ...
            console.warn(`[Runtime][${runTag}] Compaction attempt ${attempt + 1}: ` +
            ...
            if (attempt < COMPACTION_MAX_RETRIES) {
                ...
                messages.push({
                    role: 'user',
                    content: 'Your output is missing the required H1 headings. ' +
        ...
        if (lastOutput.trim().length > 0) {
            console.warn(`[Runtime][${runTag}] Compaction retries exhausted, ` +
 ...
 */
function buildFallbackCompactionSummary(content) {

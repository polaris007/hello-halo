 ...
 */
export function createAppRuntimeService(deps) {
    ...
    function buildScheduleTriggerContext(job, app) {
        const subId = job.metadata?.subscriptionId || 'unknown';
    ...
    function buildEventTriggerContext(eventType, eventPayload, app) {
        return {
            type: 'event',
    ...
    function buildManualTriggerContext(app) {
        return {
            type: 'manual',
    ...
    function buildEscalationTriggerContext(app, originalQuestion, response) {
        return {
    ...
    function broadcastAppStatus(appId) {
        try {
            const state = service.getAppState(appId);
            broadcastToAll('app:status_changed', { appId, state: state });
    ...
    async function executeWithConcurrency(app, trigger) {
        ...
        try {
            ...
            if (result.outcome !== 'error') {
                try {
                    ...
                    if (existingEntries.length === 0) {
                        ...
                        broadcastToAll('app:activity_entry:new', { appId: app.id, entry: fallbackEntry });
            ...
            if (outcome === 'error') {
                ...
                if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
                    console.warn(`[Runtime] Auto-pausing app=${app.id}: ${consecutiveErrors} consecutive errors`);
                    try {
        ...
        try {
            ...
            for (const entry of pendingEscalations) {
                ...
                const timeoutLabel = timeoutHours >= 24
                    ...
                    : `${timeoutHours} hour(s)`;
                console.log(`[Runtime] Escalation timed out: app=${app.id}, entry=${entry.id}, ` +
                ...
                broadcastToAll('app:activity_entry:new', { appId: app.id, entry: errorEntry });
                ...
                // 4. Notify the user
                notifyAppEvent(app.spec.name, `Escalation timed out — no response within ${timeoutLabel}.`, { appId: app.id });
            }
        }
        catch (err) {
    ...
    function subscriptionToSchedulerJob(app, sub, index) {
    ...
    function subscriptionToEventFilter(sub) {
        switch (sub.source.type) {
            case 'file': {
                const filter = { types: ['file.*'] };
                const rules = [];
            ...
            case 'webhook': {
                const filter = { types: ['webhook.received'] };
    ...
    const service = {
        ...
        async activate(appId) {
            ...
            for (let i = 0; i < subscriptions.length; i++) {
                ...
                if (jobCreate) {
                    ...
                    if (existingJob) {
                        const scheduleChanged = JSON.stringify(existingJob.schedule) !== JSON.stringify(jobCreate.schedule);
                        if (scheduleChanged) {
            ...
            if (state.schedulerJobIds.length > 0 || state.eventUnsubscribers.length > 0) {
                state.keepAliveDisposer = background.registerKeepAliveReason(`${KEEP_ALIVE_REASON}:${appId}`);
            }
            activations.set(appId, state);
            console.log(`[Runtime] App activated: ${appId}, ` +
        ...
        syncAppSchedule(appId) {
            const state = activations.get(appId);
            if (!state)
                return; // Not activated — nothing to sync
            const app = appManager.getApp(appId);
            if (!app)
                return;
            const subscriptions = app.spec.subscriptions ?? [];
        ...
        // ── Escalation ──────────────────────────────────
        async respondToEscalation(appId, entryId, response) {
        ...
        // ── Activity Queries ────────────────────────────
        getActivityEntries(appId, options) {
            return store.getEntriesForApp(appId, options);
        },
        getRun(runId) {
            return store.getRun(runId);
        },
        getRunsForApp(appId, limit) {
    ...
    scheduler.onJobDue(async (job) => {
        const appId = job.metadata?.appId;
        if (!appId) {
    ...
    appManager.onAppStatusChange((appId, _oldStatus, newStatus) => {

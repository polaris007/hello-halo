...
import { getApiCredentials, getApiCredentialsForSource, getWorkingDir, getHeadlessElectronPath, sendToRenderer, setMainWindow } from '../../services/agent/helpers';
...
import { getOrCreateV2Session, closeV2Session, createSessionState, registerActiveSession, unregisterActiveSession, activeSessions } from '../../services/agent/session-manager';
 ...
 */
export function getAppChatConversationId(appId) {
 ...
 */
export async function sendAppChatMessage(mainWindow, request) {
    ...
    try {
        ...
        const v2Session = await getOrCreateV2Session(spaceId, conversationId, sdkOptions, undefined, // no sessionId for resumption
        { aiBrowserEnabled: usesAIBrowser }, workDir);
        ...
        await processStream({
            ...
            callbacks: {
                onComplete: (streamResult) => {
                    ...
                    console.log(`[AppChat][${appId}] Stream complete: ` +
 ...
 */
export async function stopAppChat(appId) {
    const conversationId = getAppChatConversationId(appId);
 ...
 */
export function isAppChatGenerating(appId) {
    const conversationId = getAppChatConversationId(appId);
 ...
 * @param appId - App ID
 */
export function loadAppChatMessages(spacePath, appId) {
 ...
 * @param appId - App ID
 */
export function getAppChatSessionState(appId) {
    const conversationId = getAppChatConversationId(appId);
 ...
 * @param appId - App ID
 */
export function cleanupAppChatBrowserContext(appId) {
    const conversationId = getAppChatConversationId(appId);
    const ctx = scopedContexts.get(conversationId);
    if (ctx) {
        ctx.destroy();
...
//# sourceMappingURL=app-chat.js.map
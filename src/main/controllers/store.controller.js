 ...
 */
import { listApps, getAppDetail, installFromStore, refreshIndex, checkUpdates, getRegistries, addRegistry, removeRegistry, toggleRegistry, } from '../store';
import { getAppManager } from '../apps/manager';
const ALLOWED_APP_TYPES = new Set(['automation', 'skill', 'mcp', 'extension']);
// ============================================================================
// List / Query
// ============================================================================
/**
 ...
 */
export async function listStoreApps(query) {
    try {
 ...
 */
export async function getStoreAppDetail(slug) {
    try {
        if (!slug) {
 ...
 */
export async function installStoreApp(slug, spaceId, userConfig) {
    try {
        if (!slug) {
            return { success: false, error: 'App slug is required' };
        }
 ...
 */
export async function refreshStoreIndex() {
    try {
        await refreshIndex();
 ...
 */
export async function checkStoreUpdates() {
    try {
 ...
 */
export function getStoreRegistries() {
    try {
 ...
 */
export function removeStoreRegistry(registryId) {
    try {
        if (!registryId) {
 ...
 */
export function toggleStoreRegistry(registryId, enabled) {
    try {
        if (!registryId) {
            return { success: false, error: 'Registry ID is required' };
        }
...
function normalizeStoreQuery(query) {
    ...
    if (query.type && ALLOWED_APP_TYPES.has(query.type)) {

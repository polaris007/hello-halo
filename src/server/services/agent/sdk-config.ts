...
export interface BaseSdkOptionsParams {
  ...
  workDir: string
  /** Path to Node.js executable (replaces Electron binary in server environment) */
  nodePath: string
  /** Space ID */
  spaceId: string
  /** Conversation ID */
...
export function buildSdkEnv(params: SdkEnvParams): Record<string, string | number> {
  const env: Record<string, string | number | undefined> = {
    ...
    // Server environment: run as Node.js process (no Electron)
    // NODE_OPTIONS can be used for debugging if needed
    ...
    CLAUDE_CONFIG_DIR: (() => {
      const configDir = path.join(getUserDataPath(), 'claude-config')
